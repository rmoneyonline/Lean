...
{
    /// <summary>
    /// Brokerage Model implementation for XTS
    /// </summary>
    public class XtsBrokerageModel : DefaultBrokerageModel
    {
        ...
        /// <summary>
        /// Initializes a new instance of the <see cref="XtsBrokerageModel"/> class
        /// </summary>
        /// <param name="accountType">The type of account to be modelled, defaults to <see cref="AccountType.Margin"/></param>
        public XtsBrokerageModel(AccountType accountType = AccountType.Margin) : base(accountType)
        {
        }
        ...
        public override bool CanSubmitOrder(Security security, Order order, out BrokerageMessageEvent message)
        {
                ...
                security.Type != SecurityType.IndexOption)
            {
                message = new BrokerageMessageEvent(BrokerageMessageType.Warning, "NotSupported",
                    Invariant($"The {nameof(XtsBrokerageModel)} does not support {security.Type} security type.")
                );
            ...
            {
                message = new BrokerageMessageEvent(BrokerageMessageType.Warning, "NotSupported",
                    Invariant($"The {nameof(XtsBrokerageModel)} does not support {order.TimeInForce.GetType().Name} time in force.")
                );
        ...
        public override decimal GetLeverage(Security security)
        {
            ...
            if (security.Type == SecurityType.Equity || security.Type == SecurityType.Future || 
                security.Type == SecurityType.Option || security.Type == SecurityType.Index || security.Type == SecurityType.IndexOption) 
            {
                return _maxLeverage;
            }
        ...
        {
            var symbol = Symbol.Create("BANKNIFTY", SecurityType.Index, Market.India);
            return SecurityBenchmark.CreateInstance(securities, symbol);
        }

        /// <summary>
        /// Provides XTS fee model
        /// </summary>
        /// <param name="security"></param>
        /// <returns></returns>
        public override IFeeModel GetFeeModel(Security security)
        {
            return new XTSFeeModel();
        }
        ...
        private static IReadOnlyDictionary<SecurityType, string> GetDefaultMarkets()
        {
            ...
            map[SecurityType.Index]  = Market.India;
